...
```{r raster-extraction}
terra::extract(immigrants_cologne, terra::vect(fancy_points))
```
...
fancy_points <-
  fancy_points %>% 
  dplyr::mutate(
    immigrant_rate_value = terra::extract(immigrant_rate, terra::vect(.))
  )
...
dummy_cologne <- germans_cologne
dummy_cologne[dummy_cologne > 0] <- 3
...
## Carbon monoxide in the Ukraine after the start of the war

```{r gxc-pic, echo = FALSE, out.width = "85%"}
woRkshoptools::include_picture("combined_plot.png")
```
...
kernel_densities <- 
  ...
  sf::as_Spatial() %>%
  as("ppp") %>% 
  spatstat.core::density.ppp(sigma = 250) %>% 
  terra::rast()
