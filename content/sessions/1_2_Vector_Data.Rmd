...
Every object is defined by (a) longitude (x) and latitude (y) values(s).
]
...
The geometries of vector data hold three information:
...
We want to import the shapefile the administrative borders of the German states (*Bundesländer*) called `VG250_STA.shp`.
...
Since it would be a little bit boring to work with just one attribute, there's an extra table with more attributes called *attributes_districts.csv*.
...
sf::st_write(
  german_districts_enhanced, 
  "./participant_materials/districts_enhanced.shp"
)
...
konstanz <-
  german_districts_enhanced %>% 
  dplyr::filter(district_id == "08335")

# filter surrounding districts, append with Cologne data and select the Covid column
konstanz_sur <-
  german_districts_enhanced %>%
  dplyr::filter(lengths(sf::st_touches(., konstanz)) > 0) %>% 
  dplyr::bind_rows(konstanz) %>%   
  dplyr::select(death_rate)
