...
cologne_pt_stops_densities <- 
  ...
  sf::as_Spatial() %>%
  as("ppp") %>% 
  spatstat.core::density.ppp(sigma = 500) %>% 
  terra::rast()
...
```{r voting-districts-cologne}
voting_districts_cologne <-
  glue::glue(
    ...
    =&geometryPrecision=&outSR=4326&havingClause=&returnIdsOnly=false&\\
    ...
    rangeValues=&quantizationParameters=&featureEncoding=esriDefault&f=geojson"
  ) %>% 
...
With just one 'simple' command we can retrieve geospatial data about Cologne's voting districts `R`. Not too bad, right?

.pull-left[
```{r voting-districts-cologne-plot, eval = FALSE}
tm_shape(voting_districts_cologne) +
  tm_borders()
```
]
...
.pull-right[
```{r voting-districts-cologne-plot-exec, ref.label = "voting-districts-cologne-plot", echo = FALSE}
```
]
...
```{r cologne-raster}
cologne_raster <-
  tmaptools::read_osm(voting_districts_cologne) %>% # it's a stars object
  terra::rast()
...
As these data are images, they are perfect to be used as background maps when mapping other geospatial attributes, such as our voting districts data.
...
tm_shape(cologne_green) +
  tm_raster("values", palette = "Greens")
```
