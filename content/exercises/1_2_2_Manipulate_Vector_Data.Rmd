...
We want a first descriptive visual of the distribution of Covid-19 deaths in Konstanz and the surrounding districts. 
Calculate the number of Covid-19 deaths (`death_rate`) by population (`population`) and multiply with 100,000. 

Filter Konstanz (`district_id == "08335"`), find the surrounding districts, and plot Konstanz and its surrounding districts.
```

```{block, opts.label = "clues"}
You can use the dplyr function `sf::bind_rows()` to combine the two spatial objects, "Konstanz" and "Konstanz Surroundings".
```
...
Save your data set of Konstanz and its surrounding districts as an ESRI Shapefile. 
```
...
sf::st_write(
  cologne_sur, 
  dsn = "./data/participant_material/cologne_covid19_epsg3035.shp", 
  delete_layer = TRUE
)
```
